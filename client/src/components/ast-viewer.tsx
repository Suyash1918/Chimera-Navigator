...
import { ChevronDown, ChevronRight, FileCode, Tag, Type, Copy, Expand } from "lucide-react";
import { cn } from "@/lib/utils";
import { useQuery } from "@tanstack/react-query";
import type { ASTNode } from "@shared/schema";
...
export function ASTViewer() {
  ...
  const renderNode = (node: ASTNode, depth = 0) => {
    ...
    return (
      <div key={node.astPath} className="space-y-1">
        <div 
          className={cn(
            "bg-white rounded-md p-3 border border-gray-200 hover:bg-blue-50 hover:translate-x-1 transition-all duration-200 cursor-pointer",
        ...
        {isExpanded && hasChildren && (
          <div className="space-y-1">
            ...
            {node.definition?.elements?.map((element, index) => renderNode(element, depth + 1))}
          </div>
        )}
      </div>
    );
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-lg font-semibold text-gray-900">AST Structure & Paths</h3>
        <div className="flex items-center space-x-2">
          <Button variant="outline" size="sm" onClick={expandAll}>
            <Expand className="mr-1" size={12} />
            Expand All
          </Button>
      ...
      <div className="bg-gray-50 rounded-lg p-4 max-h-96 overflow-y-auto">
        <div className="space-y-2">
          {astData && astData.length > 0 ? (
            astData.map((node: ASTNode, index: number) => renderNode(node, 0))
          ) : (
